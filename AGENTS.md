...
Collection of **20 standalone Windmill scripts** (no tests, no CI, no shared lib).
...
**`README.md` only documents `stock-discrepancy/` tools — ignore it for scope.**
...
| `XMS_LEGACY_DSN` | `stock-discrepancy/xmsc-xmsl`, `xmsl-reseller`, `courier`, `office`, `order-not-sync`, `payment-cash-not-sync` |
...
- **`dblink`** used in `point/`, `payment/`, `order-not-sync`, `xmsl-reseller`, and `voucher` for cross-DB queries from Catalyst.
